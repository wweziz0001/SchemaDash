# ChartDB to SchemaDash Rebrand Audit

This document tracks the repository-wide rename from `ChartDB` to `SchemaDash` and classifies the affected areas by migration risk.
...
- Frontend context and hook modules under `src/context/chartdb-context/*` and `src/hooks/use-chartdb.ts`
- Internal symbol names such as `ChartDBProvider`, `useChartDB`, `CHARTDB_BACKUP_FORMAT`, and branding helpers
...
- `CHARTDB_*` backend environment variables
- `VITE_HIDE_CHARTDB_CLOUD` and related runtime config keys
- Cookie names, local data directories, SQLite filenames, and backup envelope identifiers
- Public headers such as `x-chartdb-share-token`
...
3. Introduce `SCHEMADASH_*` environment variables as the preferred names and support legacy `CHARTDB_*` aliases during migration.
...
- Self-hosted deployments may still export legacy `CHARTDB_*` variables.
