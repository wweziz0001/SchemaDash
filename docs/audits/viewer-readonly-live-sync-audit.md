...
1. `SchemaDashProvider` derives `readonly` from the route-only prop and diff mode.
...
1. Resolve diagram access from the saved/persisted diagram metadata inside `SchemaDashProvider`.
