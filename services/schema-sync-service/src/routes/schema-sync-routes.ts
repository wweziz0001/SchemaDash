...
export const registerSchemaSyncRoutes = (
    ...
    app.get('/api/connections/:id', async (request, reply) => {
        ...
        const connection = context.connectionsService.getConnection(params.id);
        if (!connection) {
    ...
    app.get('/api/schema/snapshots/:id', async (request, reply) => {
        const params = request.params as { id: string };
        const snapshot = context.metadataRepository.getSnapshot(params.id);
        if (!snapshot) {
    ...
    app.get('/api/schema/plans/:id/latest-audit', async (request, reply) => {
        const params = request.params as { id: string };
        const audit = context.metadataRepository.getLatestAuditForChangePlan(
            params.id
        );
        if (!audit) {
