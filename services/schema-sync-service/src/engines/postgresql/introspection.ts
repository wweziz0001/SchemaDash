...
import { createPostgresqlClient } from './connection.js';
import type { SchemaSyncIntrospectionInput } from '../types.js';
...
export const normalizePostgresqlStringArray = (
...
export const introspectPostgresqlSchema = async ({
    secret,
    schemas,
}: SchemaSyncIntrospectionInput): Promise<CanonicalSchema> => {
    ...
    try {
        ...
        const tables = tablesResult.rows.map<CanonicalTable>((row) => {
            ...
            const primaryKey: CanonicalPrimaryKey | null = primaryKeyRow
                ? {
                      ...
                      columnIds: normalizePostgresqlStringArray(
            ...
            const foreignKeys = fkResult.rows
                ...
                .map<CanonicalForeignKey>((foreignKey) => ({
                    ...
                    referencedColumnNames: normalizePostgresqlStringArray(
        ...
        return {
            ...canonicalSchema,
            fingerprint: hashCanonicalSchema(canonicalSchema),
        };
    } finally {
        await client.end();
    }
